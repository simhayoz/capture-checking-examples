...
object OriginalWebSocketsFilter extends cask.MainRoutes {
  ...
  @cask.get("/")
  def hello(filter: String = "") =
    doctype("html")(
    html(
      head(
        link(rel := "stylesheet", href := bootstrap),
        script(src := "/static/app.js"),
        // Pass the `filter` string base64 encoded to avoid XSS attacks
        script(raw(s"""subscribeSocket(atob("${java.util.Base64.getEncoder.encodeToString(filter.getBytes)}"))"""))
      ),
      body(
        div(cls := "container")(
          h1("Scala Chat!"),
